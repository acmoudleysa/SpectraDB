from spectradb.dataloaders import FluorescenceDataLoader, NMRDataLoader
...
class TestNMRDataLoader:
    ...
    def test_NMR_metadata(self):
        ...
        assert self.dataloader.metadata == {
            "Measurement Date": datetime.fromtimestamp(
                os.path.getmtime(self.txt_file)).strftime("%Y-%m-%d"),
...
class TestFluorescenceDataLoader():
    ...
    def test_FL_data(self):
        for objs in [self.dataloader, self.dataloader_edge_case]:
            assert_array_almost_equal(objs.data['S1'],
                                      [[2.941176414, 2.915452003, 0],
                                       [2.673796892, 4.950494766, -4.889975548]])  # noqa: E501

            assert_array_almost_equal(objs.data['S2'],
                                      [[2.958580017, 8.902077675, 0],
                                       [4.866179943, 0, -7.211538315]])

            assert_array_almost_equal(objs.data['S3'],
                                      [[2.949852467, 0, 0],
                                       [4.819277287, 7.462686539, -2.347417831]])  # noqa: E501

            assert_array_almost_equal(objs.data['S4'],
                                      [[18.34862328, 0, 0],
    ...
    def test_FL_metadata(self):
        for objs, file, name in zip([self.dataloader, self.dataloader_edge_case],  # noqa: E501
        ...
        assert self.dataloader_edge_case.metadata["S1"] == {
            "Measurement Date": datetime.fromtimestamp(
                os.path.getmtime(self.edge_case)).strftime("%Y-%m-%d"),
        ...
        assert self.dataloader.metadata["S1"] == {
            "Measurement Date": datetime.fromtimestamp(
                os.path.getmtime(self.csv_file)).strftime("%Y-%m-%d"),
            "Filename": "Test.csv",
            "Sample name": "Sample1",
            "Internal sample code": None,
            "Collected by": None,
    ...
    def test_delete_measurements(self):
        ...
        assert all("S2" not in d for d in [self.dataloader.data,
                                           self.dataloader.metadata,
                                           self.dataloader._sample_id_map])
    ...
    def test_add_metadata(self):
        ...
        # This is important because the df won't be
        # changed until the user calls the property `df`.
        assert_equal(
        ...
        # since we are using df now, the property calls the
        # internal _create dataframe and updates the dataframe.

        # This is an intended behavior because spectradb uses
        # dict and only creates dataframe when users asks for it.
